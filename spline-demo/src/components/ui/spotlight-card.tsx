...
// Injected once — no background-attachment:fixed so transforms don't break it
...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  useEffect(() => {
    const syncPointer = (e: PointerEvent) => {
      ...
      // --xp / --yp on :root → used for hue shift across viewport
      document.documentElement.style.setProperty("--xp", (x / window.innerWidth).toFixed(2));
      document.documentElement.style.setProperty("--yp", (y / window.innerHeight).toFixed(2));

      // --x / --y relative to this card → gradient follows cursor per-card
      if (cardRef.current) {
  ...
  const inlineStyles: React.CSSProperties & Record<string, string | number> = {
    ...
    // --hue reads --xp from :root via CSS cascade
